...
export default function App () {
  ...
  const logEvent = useCallback(label => {
    setEvents(prev =>
      [
        { id: `${Date.now()}-${Math.random()}`, label, at: nowLabel() },
        ...prev
      ].slice(0, MAX_EVENTS)
    )
  }, [])
  ...
  return (
    <div className='demo'>
      <header className='demo__header'>
        ...
        <p>
          A production-grade, reusable HLS video player built with Vite, React,
          ReactPlayer and hls.js.
        </p>
      </header>
      ...
      <main className='demo__player-col'>
        <VideoPlayer
          ...
          onQualityChange={info => {
            setCurrentQuality(
              info.auto ? `Auto (${info.height}p)` : `${info.height}p`
            )
            logEvent(
              `quality → ${info.auto ? 'Auto' : `${info.height}p`}${
          ...
          onSubtitleChange={info => {
            setCurrentSubtitle(
              info.id === -1 ? 'Off' : info.label || `Track ${info.id + 1}`
            )
            logEvent(
              `subtitles → ${info.id === -1 ? 'off' : info.label || info.id}`
            )
          }}
        />
        ...
        <section className='demo__stream-info'>
          <div>
            ...
            <span className='demo__value demo__value--url' title={streamUrl}>
              {streamUrl}
            </span>
          </div>
          <div>
      ...
      <aside className='demo__panel'>
            ...
            {DEMO_STREAMS.map(s => (
              <button
                ...
                className={`demo__chip${
                  streamUrl === s.url ? ' demo__chip--active' : ''
                }`}
                onClick={() => {
                  setPendingUrl(s.url)
                  setStreamUrl(s.url)
        ...
        <div className='demo__field demo__field--row'>
          ...
          <div className='demo__toggle-group'>
            <button
              type='button'
              className={`demo__toggle${!debug ? ' demo__toggle--active' : ''}`}
        ...
        <div className='demo__field demo__field--row'>
          ...
          <div className='demo__toggle-group'>
            <button
              type='button'
              className={`demo__toggle${
                !adsEnabled ? ' demo__toggle--active' : ''
              }`}
        ...
        <div className='demo__field'>
          ...
          <ul className='demo__events'>
            {events.length === 0 && (
              <li className='demo__events-empty'>
                No events yet — interact with the player.
              </li>
            )}
            {events.map(evt => (
              <li key={evt.id}>
