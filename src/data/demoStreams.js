...
function buildDemoThumbnailSprite () {
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < columns; col += 1) {
      ...
      cells += `<rect x="${x}" y="${y}" width="${tileWidth}" height="${tileHeight}" fill="${
        colors[index % colors.length]
      }"/>`
      cells += `<text x="${x + tileWidth / 2}" y="${
        y + tileHeight / 2 + 5
      }" font-family="sans-serif" font-size="16" fill="#fff" text-anchor="middle">${index}</text>`
    }
  }
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${
    columns * tileWidth
  }" height="${rows * tileHeight}">${cells}</svg>`
